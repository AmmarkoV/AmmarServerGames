...
void * prepare_view_content_callback(struct AmmServer_DynamicRequest  * rqst)
{
  ...
  if (boardRendering!=0)
  {
    ...
    strcat(boardRendering,"<center>\n");
    strcat(boardRendering," <div class=\"board\">\n");
    ...
    for (y=0; y<height; y++)
    {
      ...
      for (x=0; x<width; x++)
      {
         strcat(boardRendering,"     <td width=90>\n");
          if ( board[i]==1 ) { snprintf(buf,4096,"      <img src=\"textures/circle.png\" width=90>\n",i); } else
          if ( board[i]==2 ) { snprintf(buf,4096,"      <img src=\"textures/x.png\" width=90>\n",i);      } else
                             {
...
void * prepare_move_content_callback(struct AmmServer_DynamicRequest  * rqst)
{
  ...
  if ( _GET(default_server,rqst,"s",moveRequested,128) )
   ...
   if (move<width*height)
   {
    ...
    if (activePlayer==1) { activePlayer=2; } else
...
void init_dynamic_content()
{
  ...
  if (! AmmServer_AddResourceHandler(default_server,&indexPage,"/index.html",webserver_root,150000,0,&prepare_view_content_callback,DIFFERENT_PAGE_FOR_EACH_CLIENT) )  { AmmServer_Warning("Failed adding stats page\n"); }
